function [ population_out ] = mutation( population, fitness_mat, givens )
%UNTITLED8 Summary of this function goes here
%   Detailed explanation goes here
prob_mat = cell(1,size(population,2));
nMutations = 5;
mutation_mat = cell(nMutations,size(population,2));

n = sqrt(size(population{1},2));
m = n^2;

population = repmat(population,[nMutations 1]);
...
prob_mat = repmat(prob_mat,[nMutations 1]);

for i = 1:size(population,2)
    for t = 1:nMutations
        mutation_mat{t,i} = zeros(size(prob_mat{i}));
        
        for i2 = 1 :size(mutation_mat{i},1)
            for j2 = 1 :size(mutation_mat{i},2)
                x=rand;
                if x<prob_mat{t,i}(i2,j2)
                    mutation_mat{t,i}(i2,j2)=1;
                end
            end
        end
    end
...
for i = 1:size(population,2)
    for t = 1:nMutations
        mutation_mat{t,i} = wrapSudokuToGridArrays(mutation_mat{t,i}).*~givens;
    end
end
...
for j = 1:size(population,2)
    ...
    [~,I]=min(net_fitness);
    population_out{j} = population{I,j};
end
